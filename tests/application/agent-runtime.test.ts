...
describe('single procurement agent', () => {
      ...
      .mockResolvedValueOnce(
        new Response(
          JSON.stringify({
            id: 'resp-1',
            output: [
              { type: 'function_call', call_id: 'call-1', name: 'get_event', arguments: '{"eventId":"wedding"}' },
            ],
          }),
  ...
  it('prepares the next supplier order through the agent without mutating the cart', async () => {
    const ids = idSequence();
    const planning = createDemoPlanning(undefined, ids);
    const gateway = new MockSupplierGateway();
    const supplierOrders = createSupplierOrders(planning, gateway, ids);
    const tools = createTools(planning, ids, supplierOrders);
    const agent = new AgentRuntime(new LocalAgentModel(demoIngredients), tools, ids);

    const turn = await agent.run('Підготуй наступне замовлення постачальнику', 'uk');

    expect(turn.trace.map((item) => item.name)).toEqual(['get_procurement_plan', 'prepare_supplier_order']);
    expect(turn.supplierOrder?.status).toBe('needs_substitution');
    const salmon = turn.supplierOrder?.lines.find((line) => line.ingredientId === 'salmon');
    expect(salmon?.selectedProduct).toBeUndefined();
    expect(turn.message).toContain('Кошик ще не змінено');
    expect((await gateway.getCart()).lines).toHaveLength(0);

    const substituted = await supplierOrders.approveSubstitution(
      turn.supplierOrder!.id,
      'salmon',
      salmon!.replacements[0].id,
      'uk',
    );
    expect(substituted.status).toBe('ready_for_cart');
    expect((await gateway.getCart()).lines).toHaveLength(0);

    const reviewed = await supplierOrders.previewCart(substituted.id, 'uk');
    expect(reviewed.status).toBe('cart_preview');
    expect((await gateway.getCart()).lines).toHaveLength(0);

    const applied = await supplierOrders.applyCart(reviewed.id, 'uk');
...
function createTools(
  ...
  supplierOrders = createSupplierOrders(planning, new MockSupplierGateway(), generateId),
) {
