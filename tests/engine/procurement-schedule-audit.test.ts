...
describe("demo procurement schedule audit", () => {
  it("justifies the daily batch cadence with short-lived demand", () => {
    ...
    expect(plan.batches.every((batch) => batch.lines.some(
      (line) => (ingredientById.get(line.ingredientId)?.shelfLifeDays ?? Infinity) <= 7,
    ))).toBe(true);
  });
  ...
  it("consolidates stable goods while keeping Wedding perishables close to prep", () => {
    ...
    expect(purchasesFor("salmon").find((line) => line.coveredRequiredAt.includes("2026-09-13T08:00:00+03:00"))?.deliveryAt)
      .toBe("2026-09-12T05:00:00.000Z");
    expect(purchasesFor("chicken").find((line) => line.coveredRequiredAt.includes("2026-09-13T08:00:00+03:00"))?.deliveryAt)
      .toBe("2026-09-12T05:00:00.000Z");
    expect(purchasesFor("tomato").find((line) => line.coveredRequiredAt.includes("2026-09-13T08:00:00+03:00"))?.deliveryAt)
      .toBe("2026-09-11T05:00:00.000Z");
    expect(plan.lines.every((line) => line.coveredRequiredAt.every(
