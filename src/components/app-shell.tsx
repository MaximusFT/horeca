    ...
    <div className="min-h-screen bg-[#f4f5f2] lg:grid lg:grid-cols-[236px_minmax(0,1fr)]">
      <aside className="hidden min-h-screen border-r border-[#dfe3dc] bg-[#17231c] text-white lg:flex lg:flex-col">
        <div className="flex h-[78px] items-center gap-3 border-b border-white/10 px-6">
          <div className="grid size-9 place-items-center rounded-xl bg-[#b8edca] text-sm font-black text-[#14351f]">M</div>
          <div>
      ...
      <div className="min-w-0">
        <header className="sticky top-0 z-20 flex h-[78px] items-center justify-between border-b border-[#dfe3dc] bg-[#f8f9f6]/95 px-5 backdrop-blur md:px-8 lg:px-10">
          <div className="flex items-center gap-3">
            <div className="grid size-9 place-items-center rounded-xl bg-[#193126] text-sm font-black text-white lg:hidden">M</div>
            <div>
          ...
          <div className="flex items-center gap-3">
            ...
            <AgentLauncher />
            <div className="grid size-10 place-items-center rounded-full bg-[#dfe8e1] text-xs font-bold text-[#31503d]">MK</div>
          </div>
        </header>
        {children}
      </div>
...
export function Icon({ name, className = "" }: { name: IconName; className?: string }) {
  const paths: Record<IconName, ReactNode> = {
    ...
    procurement: <><path d="M4 7.5 12 3l8 4.5v9L12 21l-8-4.5z" /><path d="m4 7.5 8 4.5 8-4.5M12 12v9" /></>,
    events: <><rect x="3" y="5" width="18" height="16" rx="3" /><path d="M8 3v4M16 3v4M3 10h18" /></>,
    inventory: <><path d="M4 5h16v5H4zM5.5 10h13v11h-13zM9 14h6" /></>,
    spark: <><path d="m12 3 1.3 4.2L17.5 9l-4.2 1.8L12 15l-1.3-4.2L6.5 9l4.2-1.8z" /><path d="m18.5 15 .7 2.3 2.3.7-2.3.7-.7 2.3-.7-2.3-2.3-.7 2.3-.7z" /></>,
    calendar: <><rect x="3" y="5" width="18" height="16" rx="3" /><path d="M8 3v4M16 3v4M3 10h18" /></>,
    package: <><path d="M4 7.5 12 3l8 4.5v9L12 21l-8-4.5z" /><path d="m4 7.5 8 4.5 8-4.5M12 12v9" /></>,
    arrow: <><path d="M5 12h14M14 7l5 5-5 5" /></>,
  };
  return <svg aria-hidden="true" className={`size-[18px] shrink-0 fill-none stroke-current stroke-[1.8] ${className}`} viewBox="0 0 24 24" strokeLinecap="round" strokeLinejoin="round">{paths[name]}</svg>;
}
