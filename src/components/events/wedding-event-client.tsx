...
export function WeddingEventClient({
  event,
  activePlanVersion,
  menuLines,
  impact,
  ...
  return (
    <main className="px-5 py-7 md:px-8 md:py-9 lg:px-10 xl:px-12">
      <div className="mx-auto max-w-[1240px]">
        <Link href="/events" className="text-xs font-semibold text-[#5d7565]">← Back to events</Link>
        <div className="mt-6 flex flex-col justify-between gap-5 md:flex-row md:items-start">
          <div>
            <div className="flex items-center gap-2">
              <span className="rounded-full bg-[#e8f2eb] px-2.5 py-1 text-[9px] font-bold uppercase tracking-wide text-[#477257]">Confirmed</span>
            ...
            <p className="mt-2 text-sm text-[#6f7c73]">
              September {Number(event.startsAt.slice(8, 10))}, 2026 · Event starts {event.startsAt.slice(11, 16)} · Prep starts {event.prepStartsAt.slice(11, 16)}
            </p>
          </div>
        ...
        {success && <div className="mt-6 rounded-xl border border-[#cfe5d5] bg-[#f2faf4] px-4 py-3 text-sm font-medium text-[#39704b]">✓ {success}</div>}
        {error && <div className="mt-6 rounded-xl border border-[#efd2cc] bg-[#fff6f4] px-4 py-3 text-sm font-medium text-[#a65243]">{error}</div>}
...
function ImpactDrawer({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-[#132019]/35 backdrop-blur-[2px]" role="presentation">
      <section role="dialog" aria-modal="true" aria-labelledby="impact-title" className="absolute inset-y-0 right-0 flex w-full max-w-[560px] flex-col bg-[#f8f9f6] shadow-[-20px_0_60px_rgba(20,35,27,.18)]">
        <header className="border-b border-[#dde2dc] bg-white px-6 py-5">
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <h2 id="impact-title" className="mt-2 text-xl font-semibold text-[#223028]">{eventName} procurement impact</h2>
              <p className="mt-1 text-xs text-[#7d8981]">Nothing changes until you approve this candidate plan.</p>
            </div>
            <button type="button" onClick={onCancel} className="grid size-9 place-items-center rounded-full border border-[#dde2dc] text-lg text-[#65736a]">×</button>
          </div>
        </header>
        <div className="flex-1 overflow-y-auto p-6">
          ...
          <div className="mt-4 rounded-xl border border-[#d9e5f4] bg-[#f3f7fd] p-4 text-xs leading-5 text-[#536c92]">Candidate plan was recalculated from source state. Procurement lines were not edited manually.</div>
        </div>
        <footer className="flex gap-3 border-t border-[#dde2dc] bg-white p-5">
          <button type="button" onClick={onCancel} className="h-11 flex-1 rounded-xl border border-[#d7ddd7] text-sm font-semibold text-[#536158]">Cancel</button>
          <button type="button" onClick={onApply} disabled={loading} className="h-11 flex-[1.4] rounded-xl bg-[#1d5d38] text-sm font-semibold text-white disabled:opacity-50">{loading ? "Applying…" : "Apply changes"}</button>
        </footer>
      </section>
    </div>
  );
