      ...
      <main className="px-5 py-7 md:px-8 md:py-9 lg:px-10 xl:px-12">
        <div className="mx-auto max-w-[1320px]">
          <div className="flex flex-col justify-between gap-5 md:flex-row md:items-end">
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.16em] text-[#728078]">
                Shared kitchen · Plan v{activePlan.version}
              </p>
              ...
              <p className="mt-2 max-w-2xl text-sm leading-6 text-[#6d7a72]">
                What is on hand, what is confirmed to arrive and where the procurement plan must fill a gap.
              </p>
            </div>
            <Link
              href="/procurement"
          ...
          <section className="mt-7 grid grid-cols-2 gap-3 md:grid-cols-4" aria-label="Inventory status summary">
            {(Object.entries(statusStyle) as Array<[InventoryStatus, (typeof statusStyle)[InventoryStatus]]>).map(
              ([status, presentation]) => (
            ...
            <div className="overflow-x-auto">
              <table className="w-full min-w-[820px] border-collapse text-left">
                <thead className="border-b border-[#e6e9e5] bg-[#f8f9f7] text-[10px] font-semibold uppercase tracking-wide text-[#88938c]">
                  <tr>
                    <th className="px-5 py-3.5 md:px-6">Ingredient</th>
                    <th className="px-5 py-3.5 text-right">On hand</th>
                    <th className="px-5 py-3.5 text-right">Safety target</th>
                    <th className="px-5 py-3.5 text-right">Confirmed incoming</th>
                    <th className="px-5 py-3.5">Next requirement</th>
                    <th className="px-5 py-3.5 text-right md:px-6">Status</th>
                  </tr>
                </thead>
                    ...
                    <tr key={row.ingredientId} className="hover:bg-[#fbfcfa]">
                      ...
                      <td className="px-5 py-4">
                        <p className="text-xs font-semibold text-[#46544b]">
                          {row.nextNeedAt ? formatDateTime(row.nextNeedAt) : 'No requirement'}
                        </p>
                        {row.nextNeedAt && (
                      ...
                      <td className="px-5 py-4 text-right md:px-6">
                        <span
                          className={`inline-flex rounded-full px-2.5 py-1 text-[9px] font-bold uppercase tracking-wide ${statusStyle[row.status].className}`}
                        >
