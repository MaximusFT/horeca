    ...
    <AppShell activeKey="procurement">
      <main className="px-5 py-7 md:px-8 md:py-9 lg:px-10 xl:px-12">
        <div className="mx-auto max-w-[1320px]">
          <Link href="/procurement" className="text-xs font-semibold text-[#5d7565]">{dictionary.procurementBatch.backToProcurement}</Link>
          <div className="mt-6 flex flex-col justify-between gap-5 md:flex-row md:items-end"><div><div className="flex items-center gap-2"><span className="rounded-full bg-[#fff0d7] px-2.5 py-1 text-[9px] font-bold uppercase tracking-wide text-[#94651f]">{dictionary.procurementBatch.plannedStatus}</span><span className="text-xs text-[#849087]">{dictionary.procurementBatch.planLabel(activePlan.version)}</span></div><h1 className="mt-3 text-[34px] font-semibold tracking-[-0.04em] text-[#18251d]">{dictionary.procurementBatch.deliveryTitle(prettyDate(batch.deliveryOn, dictionary.locale))}</h1><p className="mt-2 text-sm text-[#6f7c73]">{dictionary.procurementBatch.targetArrival(batch.deliveryAt.slice(11, 16), batch.lines.length)}</p></div><MockSupplierFlow batchId={batch.id} locale={locale} /></div>
          <div className="mt-7"><ProcurementBatchTable lines={lines} locale={locale} /></div>
        </div>
      </main>
    </AppShell>
  );
...
function prettyDate(date: string, locale: string): string {
  return new Intl.DateTimeFormat(locale === "uk" ? "uk-UA" : "en", { day: "numeric", month: "long", timeZone: "Europe/Kyiv" }).format(new Date(`${date}T12:00:00+03:00`));
}
