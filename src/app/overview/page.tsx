        ...
        <div className="mx-auto max-w-[1480px]">
          <div className="flex flex-col justify-between gap-5 md:flex-row md:items-end">
            <div>
              ...
              <p className="mt-2 max-w-2xl text-sm leading-6 text-[#65736a]">Restaurant demand, {summary.eventCount} events, stock and incoming supply are reconciled into one dated procurement plan.</p>
            </div>
          ...
          <section className="mt-7 grid gap-3 sm:grid-cols-2 xl:grid-cols-4" aria-label="Business planning summary">
            <BusinessMetricCard
              ...
              meta={summary.nextPeakDay ? `Next peak · ${summary.nextPeakDay.weekday} Sep ${summary.nextPeakDay.dayNumber} · ×${summary.nextPeakDay.loadFactor.toFixed(2)}` : "Normal service calendar active"}
              accent="green"
            />
            <BusinessMetricCard
              ...
              meta={summary.largestEvent ? `Largest · ${summary.largestEvent.name} · ${summary.largestEvent.guestCount} guests` : "No events scheduled"}
              accent="blue"
            />
            <BusinessMetricCard
              eyebrow="Combined procurement"
              value={`${summary.batchCount} planned deliveries`}
              detail={summary.upcomingBatches[0] ? `Next · Sep ${Number(summary.upcomingBatches[0].deliveryOn.slice(-2))}` : "No delivery required"}
              meta={summary.upcomingBatches[0] ? `${summary.upcomingBatches[0].lines.length} ingredients in next batch` : "Demand currently covered"}
              accent="neutral"
            />
            <BusinessMetricCard
              eyebrow="Attention"
          ...
          <section className="mt-6 overflow-hidden rounded-2xl border border-[#dfe3dc] bg-white shadow-[0_1px_2px_rgba(24,37,29,.03)]" aria-label="How demand becomes procurement">
            <div className="grid divide-y divide-[#e8ebe7] md:grid-cols-2 md:divide-x md:divide-y-0">
              <div className="px-5 py-5 md:px-6">
                <p className="text-[10px] font-bold uppercase tracking-[0.16em] text-[#4c7c5b]">Restaurant operations</p>
                <p className="mt-2 text-lg font-semibold text-[#26342b]">Regular daily demand</p>
                <p className="mt-1 text-xs leading-5 text-[#77837b]">A {summary.operatingDayCount}-day service calendar keeps the kitchen supplied even when no catering event is scheduled.</p>
              </div>
              <div className="px-5 py-5 md:px-6">
                <p className="text-[10px] font-bold uppercase tracking-[0.16em] text-[#526bb2]">Events & catering</p>
                <p className="mt-2 text-lg font-semibold text-[#26342b]">{summary.eventCount} events · {summary.guestTotal} guests</p>
                <p className="mt-1 text-xs leading-5 text-[#77837b]">Menus and guest counts add dated demand to the same kitchen and shared inventory.</p>
              </div>
            </div>
            <div className="flex flex-col justify-between gap-4 bg-[#1d3126] px-5 py-5 text-white md:flex-row md:items-center md:px-6">
              ...
              <div className="flex items-center gap-3 text-xs text-white/65"><span>Shared stock</span><span>+</span><span>Confirmed incoming</span><Icon name="arrow" className="text-[#a9daba]" /></div>
            </div>
          </section>

          <section className="mt-6 rounded-2xl border border-[#dfe3dc] bg-white shadow-[0_1px_2px_rgba(24,37,29,.03)]">
            <SectionHeader title="Where demand comes from" subtitle={`Regular restaurant operations and ${summary.eventCount} scheduled events · ${summary.guestTotal} guests`} />
          ...
          <section className="mt-6 overflow-hidden rounded-2xl border border-[#dfe3dc] bg-white shadow-[0_1px_2px_rgba(24,37,29,.03)]">
            <header className="flex flex-col justify-between gap-3 border-b border-[#e5e8e3] px-5 py-5 md:flex-row md:items-center md:px-6">
              <div><h2 className="text-[15px] font-semibold text-[#1c2921]">14-day operations timeline</h2><p className="mt-1 text-xs text-[#7a877f]">Three views of the same planning horizon</p></div>
            </header>
                ...
                {summary.upcomingBatches.map((batch, index) => (
                  <div key={batch.id} className="grid gap-3 px-5 py-4 sm:grid-cols-[78px_minmax(0,1fr)_auto] sm:items-center md:px-6">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="mt-1 pl-4 text-xs text-[#7c8981]">{batch.lines.length} ingredients · covers restaurant operations and upcoming events</p>
                    </div>
            ...
            <section className="rounded-2xl border border-[#dfe3dc] bg-white shadow-[0_1px_2px_rgba(24,37,29,.03)]">
              ...
              <div className="space-y-3 p-4 md:p-5">
                {summary.attention.map((item) => {
                  ...
                  return <article key={item.id} className={`rounded-xl border p-4 ${tone}`}><div className="flex items-start gap-3"><span className="mt-1 size-2 rounded-full bg-current opacity-80" /><div className="min-w-0 flex-1"><h3 className="text-xs font-semibold text-[#26342b]">{item.title}</h3><p className="mt-1 text-[11px] leading-5 text-[#748078]">{item.description}</p><div className="mt-2 flex flex-wrap items-center justify-between gap-2"><p className="text-[10px] font-semibold uppercase tracking-wide">{item.meta}</p>{item.href && item.actionLabel && <Link href={item.href} className="text-[10px] font-bold text-current underline decoration-current/30 underline-offset-4">{item.actionLabel} →</Link>}</div></div></div></article>;
                })}
              </div>
            </section>
          </div>
...
function BusinessMetricCard({ eyebrow, value, detail, meta, accent }: { eyebrow: string; value: string; detail: string; meta: string; accent: "green" | "blue" | "neutral" | "amber" }) {
  const accentStyle = { green: "bg-[#5ead76]", blue: "bg-[#6c85d7]", neutral: "bg-[#9aa59e]", amber: "bg-[#dca04a]" }[accent];
  return <article className="rounded-2xl border border-[#dfe3dc] bg-white p-5 shadow-[0_1px_2px_rgba(24,37,29,.03)]"><div className="flex items-start justify-between gap-4"><p className="text-[10px] font-bold uppercase tracking-[0.14em] text-[#748178]">{eyebrow}</p><span className={`size-2 rounded-full ${accentStyle}`} /></div><p className="mt-4 text-xl font-semibold tracking-[-0.025em] text-[#1c2921]">{value}</p><p className="mt-1 text-sm font-medium text-[#536158]">{detail}</p><p className="mt-4 border-t border-[#edf0ec] pt-3 text-[10px] leading-4 text-[#89938d]">{meta}</p></article>;
}

function TimelineLanes({ days }: { days: OverviewDay[] }) {
  return <div className="overflow-x-auto"><div className="grid min-w-[1320px] grid-cols-[156px_repeat(14,minmax(80px,1fr))] text-center"><div className="sticky left-0 z-10 border-r border-[#e5e8e3] bg-[#f8f9f7]" />{days.map((day) => <div key={day.date} className={`border-r border-[#eef0ed] px-2 py-3 ${day.date === "2026-09-01" ? "bg-[#f1f8f3]" : "bg-[#f8f9f7]"}`}><p className="text-[9px] font-bold uppercase tracking-wide text-[#929c95]">{day.weekday}</p><p className="mt-1 text-sm font-semibold text-[#344138]">Sep {day.dayNumber}</p></div>)}<TimelineLabel title="Restaurant" subtitle="Operations" />{days.map((day) => { const style = loadStyle[day.load]; return <div key={`restaurant-${day.date}`} className={`border-r border-t border-[#eef0ed] px-2 py-4 ${day.date === "2026-09-01" ? "bg-[#f5faf6]" : "bg-white"}`}><p className={`text-[10px] font-bold uppercase ${style.text}`}>{style.label}</p><p className="mt-1 text-[9px] text-[#929c95]">×{day.loadFactor.toFixed(2)}</p><div className="mx-auto mt-2 h-1.5 w-12 overflow-hidden rounded-full bg-[#edf0ed]"><div className={`h-full rounded-full ${style.bar}`} /></div></div> })}<TimelineLabel title="Events" subtitle="& catering" />{days.map((day) => <div key={`events-${day.date}`} className={`min-h-[82px] border-r border-t border-[#eef0ed] px-1.5 py-3 ${day.date === "2026-09-01" ? "bg-[#f5faf6]" : "bg-white"}`}>{day.events.map((event) => <Link key={event.id} href={`/events/${event.id}`} className={`block rounded-lg px-2 py-2 text-left text-[9px] leading-3.5 ${event.id === "wedding" ? "bg-[#e7ecfb] text-[#3e579b] ring-1 ring-[#cfd8f3]" : "bg-[#eef2fe] text-[#455f9e]"}`}><span className="block font-semibold">{event.name}</span><span className="opacity-70">{event.guestCount} guests</span></Link>)}</div>)}<TimelineLabel title="Procurement" subtitle="Deliveries" />{days.map((day) => <div key={`procurement-${day.date}`} className={`min-h-[68px] border-r border-t border-[#eef0ed] px-1.5 py-3 ${day.date === "2026-09-01" ? "bg-[#f5faf6]" : "bg-white"}`}>{day.batchId ? <Link href={`/procurement/${day.batchId}`} className="block rounded-lg border border-[#dfe4df] bg-[#f8faf8] px-2 py-2 text-left text-[9px] text-[#5e6c63]"><span className="flex items-center gap-1.5 font-semibold"><span className="size-1.5 rounded-full bg-[#708f79]" />Delivery</span><span className="mt-0.5 block opacity-75">{day.procurementLineCount} ingredients</span></Link> : <span className="text-[9px] text-[#bdc4bf]">Covered</span>}</div>)}</div></div>;
}

function TimelineLabel({ title, subtitle }: { title: string; subtitle: string }) {
  return <div className="sticky left-0 z-10 flex min-h-[68px] flex-col justify-center border-r border-t border-[#e5e8e3] bg-[#f8f9f7] px-5 text-left"><p className="text-[10px] font-bold uppercase tracking-[0.12em] text-[#526159]">{title}</p><p className="mt-0.5 text-[9px] uppercase tracking-wide text-[#98a19b]">{subtitle}</p></div>;
}
