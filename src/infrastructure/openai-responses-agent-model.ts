...
function extractText(items: ResponsesApiOutputItem[]): string {
  return items.flatMap((item) => item.content ?? []).filter((item) => item.type === "output_text").map((item) => item.text ?? "").join("\n");
}

function isToolName(value: string): value is AgentToolName {
  return ["get_event", "get_procurement_plan", "explain_requirement", "preview_event_change", "apply_event_change"].includes(value);
}
