...
export class TursoSilpoTimeslotApprovalStore implements SilpoTimeslotApprovalStore {
  ...
  async claim(sessionId: string, approvalId: string, now: string): Promise<SilpoTimeslotApproval | undefined> {
    ...
    const payloadResult = await this.client.execute({
      sql: `SELECT encrypted_payload FROM ${TABLE_NAME} WHERE approval_id = ? AND session_id = ?`,
      args: [approvalId, sessionId],
    });
    const encryptedPayload = payloadResult.rows[0]?.encrypted_payload;
